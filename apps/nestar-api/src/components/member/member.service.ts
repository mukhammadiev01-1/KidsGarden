...
export class MemberService {
	...
	private shapeSelfUpdateInput(input: MemberUpdate): Partial<MemberUpdate> {
		// Self-update must be explicitly whitelisted to prevent role/status escalation.
		const allowedFields: (keyof MemberUpdate)[] = [
			'memberPhone',
			'memberNick',
			'memberFullName',
			'memberImage',
		...
		return update as Partial<MemberUpdate>;
	}

	public async getMember(memberId: ObjectId, targetId: ObjectId): Promise<PublicMember> {
		const search: T = {
			_id: targetId,
			memberStatus: MemberStatus.ACTIVE,
		};
	...
	public async getKindergartenAdmins(memberId: ObjectId, input: KindergartenAdminsInquiry): Promise<PublicMembers> {
