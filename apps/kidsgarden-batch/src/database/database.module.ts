...
@Module({
	imports: [
		// shu modul ichida ishlatiladigan modullar
		MongooseModule.forRootAsync({
			// MongoDB ga async tarzda ulanish
			useFactory: () => ({
				// dynamic config qaytaruvchi factory function
				// database uri ni belgilaydi
				uri:
...
export class DatabaseModule {
	constructor(
		...
		if (connection.readyState === 1) {
			// agar db muvaffaqiyatli ulangan bo'lsa
			console.log(
